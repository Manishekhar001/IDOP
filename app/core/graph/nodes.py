...
async def sql_generation_node(state: CSRAGState) -> dict:
    ...
    try:
        ...
        # Auto-execute read-only SELECT queries and return natural language answer
        is_select = sql and sql.strip().upper().startswith("SELECT")
        if is_select and is_correct:
            try:
                executor = SQLExecutor()
                results = executor.execute_and_log(query_id, question, sql)
