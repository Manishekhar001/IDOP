...
Generated by [SparseVectorService](file:///c:/Users/manis/Downloads/Agentic-AI/IDOP/app/core/sparse_vector_service.py) using the **fastembed `Qdrant/bm25`** model:

1. **Tokenization:** Uses Qdrant's BM25 tokenizer (handles casing, punctuation, compound words)
2. **IDF weighting:** Applies Inverse Document Frequency scoring using pre-trained statistical IDF lookup
3. **Output:** `SparseVector(indices=[...], values=[...])` — Qdrant's native sparse format

**Key improvements over the previous `hash()`-based approach:**
- **Deterministic across processes:** The old Python `hash()` function is randomized per process (via `PYTHONHASHSEED`), producing different sparse indices for the same text in different Python processes. The fastembed BM25 model is fully deterministic.
- **Proper IDF scoring:** Uses actual BM25 term weighting instead of raw term frequency.
- **Minimal memory footprint:** ~30-50 MB — the BM25 model is a statistical IDF lookup table, not a neural model.
- **Migration:** A one-time Qdrant collection deletion is automatically triggered on first startup to purge stale hash()-based sparse vectors.
