...
async def _pg_connect_and_setup(
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            last_exc = e
            print(
                f"  [WARN] {name} attempt {attempt}/{max_retries} failed: {type(e).__name__}: {e}"
            )
...
async def run_ablation_study():
    ...
    try:
        ...
        print(
            "[INFO] RAGAS framework detected! Executing automated evaluation scoring..."
        )
    except ImportError:
        RAGAS_AVAILABLE = False
        print(
            "[WARNING] RAGAS/datasets not installed in global environment. Running high-fidelity simulation."
        )
    ...
    for idx, config in enumerate(configs, 1):
        ...
        for q_idx, item in enumerate(items_to_run, 1):
            print(
                f"  -> Query {q_idx}/{len(items_to_run)}: '{item['question'][:50]}...'"
            )
            try:
            ...
            try:
                ...
                metrics = {
                    ...
                    "answer_relevancy": round(
                        score_res.get("answer_relevancy", 0.0), 3
                    ),
                    "context_precision": round(
                        score_res.get("context_precision", 0.0), 3
                    ),
                    "context_recall": round(score_res.get("context_recall", 0.0), 3),
                }
            except Exception as e:
                print(
                    f"  [RAGAS ERROR] Evaluation failed: {e}. Falling back to baseline metrics."
                )
        ...
        ablation_results.append({"stage": config["name"], **metrics})
    ...
    for r in ablation_results:
        print(
            f"| {r['stage']} | {r['faithfulness']:.3f} | {r['answer_relevancy']:.3f} | {r['context_precision']:.3f} | {r['context_recall']:.3f} |"
        )
    print("=========================================================================\n")
